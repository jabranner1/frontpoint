...
@Component({
  selector: 'frontpoint-telephone-input',
  ...
  implements ControlValueAccessor, MatFormFieldControl<string>, OnDestroy
{
  ...
  focused = false;
  touched = false;
  controlType = 'frontpoint-telephone-input';
  id = `frontpoint-telephone-input-${TelephoneInputComponent.nextId++}`;
  onChange = (_: any) => {};
  onTouched = () => {};
  ...
  onContainerClick(event: MouseEvent) {
    if (
      this.areaInput.nativeElement.contains(event.target as Element) ||
      this.subscriberInput.nativeElement.contains(event.target as Element) ||
      this.exchangeInput.nativeElement.contains(event.target as Element)
    )
      return;
